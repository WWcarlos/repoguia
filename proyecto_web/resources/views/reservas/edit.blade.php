...
<div class="container">
    ...
    <form action="{{ route('reservas.update', $reserva->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="usuario_id" id="usuario_id" class="form-control">
                ...
                @foreach ($users as $user)
                    <option value="{{ $user->id }}" {{ $reserva->usuario_id == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <select name="vehiculo_id" id="vehiculo_id" class="form-control">
                ...
                @foreach ($vehiculos as $vehiculo)
                    <option value="{{ $vehiculo->id }}" {{ $reserva->vehiculo_id == $vehiculo->id ? 'selected' : '' }}>{{ $vehiculo->placa }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <select name="espacio_id" id="espacio_id" class="form-control">
                ...
                @foreach ($espacios as $espacio)
                    @if ($espacio->estadoE == 'disponible' && $espacio->tipo == $reserva->vehiculo->tipo)
                        <option value="{{ $espacio->ubicacion }}" {{ $reserva->espacio->ubicacion == $espacio->ubicacion ? 'selected' : '' }}>{{ $espacio->ubicacion }}</option>
                    @endif
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <input type="date" name="fecha_entrada" id="fecha_entrada" class="form-control" value="{{ $reserva->fecha_entrada }}">
        </div>
        <div class="form-group">
            <label for="fecha_salida">Fecha Salida</label>
            <input type="date" name="fecha_salida" id="fecha_salida" class="form-control" value="{{ $reserva->fecha_salida }}">
        </div>
...
<script>
    ...
    // Trigger change event on page load to populate the vehicle and space options
